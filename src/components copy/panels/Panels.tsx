    ...
    <EditorFrame path="about.md" language="markdown">
      <div className="mb-8">
        ...
        <h1 className="mt-3 text-4xl font-bold tracking-tight sm:text-5xl">
          ...
          <span className="caret ml-1" aria-hidden>_</span>
        </h1>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div>
          ...
          <div className="rounded-lg border border-border bg-card p-5 text-sm leading-relaxed">
            <p>
              Currently focused on LLM-powered analytics, data-annotation workflows, and
              building calm, information-dense interfaces. I also lead technical execution
              for department events at MIT AOE.
            </p>
            <ul className="mt-4 space-y-2 text-muted-foreground">
              <li className="flex items-center gap-2"><MapPin className="h-4 w-4 text-primary" /> {profile.location}</li>
              <li className="flex items-center gap-2"><Mail className="h-4 w-4 text-primary" /> {profile.email}</li>
              <li className="flex items-center gap-2"><Phone className="h-4 w-4 text-primary" /> {profile.phone}</li>
            </ul>
          </div>
        </div>
          ...
          <ul className="space-y-3">
            {education.map((e) => (
              <li key={e.school} className="rounded-lg border border-border bg-card p-4">
                ...
                <div className="mt-1 font-mono text-xs text-primary">{e.period} · {e.score}</div>
              </li>
            ))}
          </ul>
        </div>
  ...
  return (
    <EditorFrame path="projects.tsx" language="tsx">
      ...
      <p className="mb-6 max-w-2xl text-sm text-muted-foreground">
        Case studies built from real coursework and internships. Each shows the problem,
        approach, stack, and current state.
      </p>
        ...
        {projects.map((p) => (
          ...
          >
            ...
            <div className="mt-4 flex flex-wrap gap-1.5">
              {p.stack.map((s) => (
                <span key={s} className="rounded border border-border bg-muted px-2 py-0.5 font-mono text-[11px]">
                  {s}
                </span>
              ))}
            </div>
            ...
            <footer className="mt-5 flex items-center justify-between text-xs text-muted-foreground">
              <span>Team of {p.team} · Mentor: {p.mentor}</span>
              <a
                href={p.link}
                target="_blank"
                rel="noreferrer"
          ...
          <li key={e.company} className="relative">
            ...
            <div className="rounded-lg border border-border bg-card p-5">
              ...
              <div className="mt-3 flex flex-wrap gap-1.5">
                {e.skills.map((s) => (
                  <span key={s} className="rounded bg-muted px-2 py-0.5 font-mono text-[11px] text-muted-foreground">
                    {s}
                  </span>
                ))}
              </div>
...
export function SkillsPanel() {
  const json = useMemo(() => {
    ...
    lines.push(<span key="0"><span className="text-syntax-keyword">const</span> <span className="text-syntax-fn">skills</span> = {"{"}</span>);
    ...
    entries.forEach(([cat, items], idx) => {
      ...
      lines.push(<span key={`c${idx}-end`}>{"  "}]{idx < entries.length - 1 ? "," : ""}</span>);
    });
  ...
  return (
    <EditorFrame path="hackathons.md" language="markdown">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {achievements.map((a) => (
          <div key={a.title} className="flex items-start gap-3 rounded-lg border border-border bg-card p-5">
            <Trophy className="h-5 w-5 shrink-0 text-primary" />
            <div>
              <div className="font-semibold">{a.title}</div>
              <div className="mt-1 text-xs text-muted-foreground">Competitive event · verified from resume</div>
            </div>
          </div>
        ))}
      </div>
      <p className="mt-6 rounded-md border border-dashed border-border bg-card/50 p-4 font-mono text-xs text-muted-foreground">
        // TODO: add event names, dates and organizers when confirmed.
      </p>
    </EditorFrame>
  );
}
    ...
    <EditorFrame path="certifications.md" language="markdown">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {certifications.map((c) => (
          <article key={c.name} className="flex flex-col rounded-lg border border-border bg-card p-5">
...
export function GitHubPanel() {
  ...
  return (
    <EditorFrame path="github.ts" language="ts">
      ...
      <p className="mt-6 rounded-md border border-dashed border-border bg-card/50 p-4 font-mono text-xs text-muted-foreground">
        // Live GitHub stats (contribution graph, followers, repo counts) intentionally
        omitted — they'll be integrated via the GitHub API once wired up. No fabricated numbers.
      </p>
    </EditorFrame>
  );
}
      ...
      <div className="overflow-hidden rounded-lg border border-border bg-card">
        ...
        >
          <div className="p-6 text-sm text-muted-foreground">
            ...
            <a href={resumeAsset.url} className="text-primary underline" target="_blank" rel="noreferrer">
  ...
  return (
    <EditorFrame path="blog.md" language="markdown">
      ...
      <ComingSoon
        ...
        body="I'm writing about LLM integrations, dataset-driven UIs, and internship lessons. Check back shortly — no ghost-written posts, no filler."
      />
    </EditorFrame>
  );
}
  ...
  return (
    <EditorFrame path="testimonials.md" language="markdown">
      ...
      <ComingSoon
        ...
        body="I'd rather ship this section with real quotes from mentors and teammates than fabricate blurbs. Reach out if you'd like to contribute one."
      />
    </EditorFrame>
  );
}
  ...
  return (
    <EditorFrame path="assistant.ai" language="ai">
      ...
      <p className="mb-4 max-w-2xl text-sm text-muted-foreground">
        A deterministic assistant grounded in Tushar's resume. It matches your question to
        verified facts — never a language model hallucination.
      </p>
    ...
    <EditorFrame path="contact.tsx" language="tsx">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <form onSubmit={submit} className="rounded-xl border border-border bg-card p-6">
          ...
          {sent && (
            <p className="mt-3 text-xs text-primary">Opening your email client… if nothing opens, email me directly.</p>
          )}
          <p className="mt-3 font-mono text-[11px] text-syntax-comment">
            // Uses your mail client. Wire EmailJS keys via Lovable Cloud when ready.
          </p>
        </form>
      </div>
    </EditorFrame>
